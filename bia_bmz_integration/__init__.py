...
def run_model_inference(bmz_model, image):
    ...
    if isinstance(model_resource, v0_5.ModelDescr):
        ...
        indices = [
            i
            for i in range(len(test_input_image.shape))
            if test_input_image.shape[i] == 1
        ]
        right_dims = np.expand_dims(new_np_array, indices)

        # convert image to tensor
        input_tensor = Tensor.from_numpy(
            right_dims, dims=tuple(model_resource.inputs[0].axes)
        )
...
def benchmark_results(correct_ch_output, ref_array):
    ...
    ssim_score = ssim_metric(
        y_pred=t_output[0, :, :, :, :], y=t_reference[0, :, :, :, :]
    )
...
def process(
    ...
    prediction, sample, inp_id, outp_id, test_input_image = run_model_inference(
        bmz_model, image
    )
